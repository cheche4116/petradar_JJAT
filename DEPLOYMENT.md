...
Esta API NestJS usa PostgreSQL. Para el entregable, la base debe estar en un servicio en linea y la API debe usar una URL publica.
...
- Tabla `lost_pets`.
- Tabla `found_pets`.
- Indices JSON para la columna `location`.

## Railway

1. Sube este repositorio a GitHub.
2. En Railway, crea un proyecto nuevo.
3. Agrega una base PostgreSQL de Railway.
...
Si el servicio de base de datos no se llama `Postgres`, ajusta la referencia `DATABASE_URL` con el nombre real que aparece en Railway, por ejemplo:
...
Debe devolver registros como `Luna` y `Rocky`, cargados desde la base PostgreSQL en linea.
...
6. Explicar brevemente que la API usa `DATABASE_URL`, migraciones y PostgreSQL remoto.
